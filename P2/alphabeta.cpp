...
/*
float AlphaBeta(Player pl,Board b,unsigned depth,bool maximizingPlayer,Move &best_move)
...
bool maximizingPlayer: boolean variable to indicate if this is the player who wants to
                       maximize or to minimize. When a player calls AlphaBeta to choose his/her
                       move, it should be passed as true (he/she is trying to get the maximum
                       value of the function) but when AlphaBeta is recursively called, it should
...
float AlphaBeta(Player pl, Board b, unsigned depth, bool maximizingPlayer, Move &best_mv, float alpha, float beta) {
  ...
  if (maximizingPlayer) {
    ...
    for (Sqlist::iterator it = sons.begin(); it!=sons.end(); it++) {
      ...
      // From that child we call AlphaBeta again with some differences:
      ...
      Move dummy_move;
      val = AlphaBeta(Opponent(pl), child, depth-1, false, dummy_move, alpha, beta);
  ...
  // Comments would be analogous. We signal only the differences:
    ...
    for (Sqlist::iterator it = sons.begin(); it != sons.end(); it++) {
      ...
      Move dummy_move;
      val = AlphaBeta(Opponent(pl), child, depth-1, true, dummy_move, alpha, beta);
